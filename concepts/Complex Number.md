...
A [[Complex Number]] is an [[Expression]] containing the [[Complex Number|Real Part]] or [[Real Number]] $a$ plus the [[Complex Number|Imaginary Part]] or another [[Real Number]] $b$ multiplied by the [[Complex Number|Imaginary Unit]] $i = \sqrt{ -1 }$ as shown [^1]
...
Given that $i = \sqrt{ -1 } \implies i^{2} = -1$, the [[Expression]] can be further simplified to the following [^1]
...
To simplify the quotient $\dfrac{{a + bi}}{c + di}$, the [[Expression]] can be multiplied with a fraction wherein the numerator and the denominator is the [[Complex Conjugate]] of the quotient's denominator so that it is technically multiplied by $1$ [^1]

$$
\dfrac{{a + bi}}{c + di} \cdot 1 = \dfrac{{a + bi}}{c + di} \cdot \dfrac{{c - di}}{c - di}
$$

This results in an [[Expression]] wherein the numerator is a [[Complex Number]] and the denominator is purely [[Real Number|Real]] [^1]
